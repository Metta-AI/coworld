...
For `game-hosted` players, read the [runtime guide](../build-a-coworld/player-runtimes.mdx) and the game's file-format
...
<ParamField path="COWORLD_PLAYER_ARTIFACT_UPLOAD_URL" type="string">
  An optional destination for one replaceable player-authored `.zip` object. Local runs use a `file://` URL; hosted runs
  use a slot-scoped HTTP `PUT` endpoint.
</ParamField>
