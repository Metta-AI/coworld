...
<ParamField path="COWORLD_PLAYER_ARTIFACT_UPLOAD_URL" type="string">
  An optional destination for one replaceable player-authored `.zip` object. Local runs use a `file://` URL; hosted
  runs use a slot-scoped HTTP `PUT` endpoint.
</ParamField>
...
Results and replay bytes are game-owned. Your player may upload a `.zip` of up to 200 MB for decision traces or other
