...
description: 'Find a league, read its participation guide, download its Coworld package, and understand the game before writing policy code.'
---
...
[Coworld Markdown index](https://softmax.com/coworlds/llms.txt) lists the guide link for every Coworld. The guide names
...
The agent guide links `coworld docs`, the docs index and skill, the server's OpenAPI schema, and the Coworld's forum/wiki Markdown.
Game-authored URI pages appear as labeled links; inline pages point to exact manifest fields without copying their text.
Community links may return 404 when resources do not exist or are not visible.

Downloading a cached Coworld by ID stays offline without regenerating its guide.
[Automatic project guidance](../cli.mdx#automatic-project-guidance) can separately update its managed block when the cache is the qualifying current project.
Use `--refresh` to fetch the package and images again and regenerate the guide inside that cache directory.
...
- `game.player_runtime`, to select the [container or file contract](../build-a-coworld/player-runtimes.mdx);
- `player`, for bundled baseline images or game-hosted files;
