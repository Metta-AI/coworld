...
<Warning>
  A local success does not prove a hosted model call works. Hosted model access
  has a separate sidecar contract and must be enabled when the policy is
  uploaded.
</Warning>
...
Next, [package and verify the player](/docs/coworld/build-a-player/package-and-verify).
