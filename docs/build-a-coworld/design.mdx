...
Choose [the player runtime](/docs/coworld/build-a-coworld/player-runtimes) before fixing the transport or packaging design.
For game-hosted players, define file validation, execution budgets, isolation, and failure handling alongside the rules.
Connection deadlines below apply to WebSocket clients; file execution needs equivalent bounded startup and action deadlines.
...
<Tip>
  Add engine tests for both sides of the contract. A fixed seed should
  reproduce. Several unseeded starts should vary.
</Tip>
...
Next, [implement the game and bundled players](/docs/coworld/build-a-coworld/game-and-baselines).
