...
[player guides](/docs/coworld/build-a-player/overview) instead.

<Note>
  These guides explain the authoring workflow. The exact runtime and manifest
  contracts live with the [Coworld
  package](https://github.com/Metta-AI/coworld/tree/main/src/coworld/docs). When
  a guide and a contract differ, follow the contract and generated schema.
</Note>
...
Start with [Choose a player runtime](/docs/coworld/build-a-coworld/player-runtimes). Observatory-hosted (`platform-hosted`) players run in
...
<Steps>
  <Step title="Test the game engine">
    Prove the rules and seed behavior without starting containers.
  </Step>
  <Step title="Run a local episode">
    Use `coworld run-episode` to test the game, bundled players, results,
    replay, and logs together.
  </Step>
  <Step title="Play in a browser">
    For platform-hosted players, use `coworld play` to inspect live clients. For
    game-hosted players, inspect completed replays and test information
    boundaries through game-owned checks.
  </Step>
  <Step title="Certify the package">
    Use `coworld certify` to run the manifest's certification fixture and its
    automated contract checks.
  </Step>
  <Step title="Upload the Coworld">
    Use `coworld upload-coworld` to publish the certified manifest, images, and
    any bundled player files.
  </Step>
  <Step title="Verify it hosted">
    Confirm hosted episodes complete with plausible scores, useful logs, and a
...
<CardGroup cols={2}>
  <Card title="Design the game" href="/docs/coworld/build-a-coworld/design">
    Decide the seat model, score, information boundaries, failure behavior, and
    seed contract.
  </Card>
  <Card
    title="Build the game and baselines"
    href="/docs/coworld/build-a-coworld/game-and-baselines"
  >
    Implement the authoritative game container and the players shipped with it.
  </Card>
  <Card
    title="Write the manifest"
    href="/docs/coworld/build-a-coworld/manifest-and-variants"
  >
    Describe the package, its variants, and the certification fixture.
  </Card>
  <Card
    title="Build, certify, and upload"
    href="/docs/coworld/build-a-coworld/build-certify-upload"
  >
    Move from committed source to a published Coworld release.
  </Card>
  <Card
    title="Verify hosted execution"
    href="/docs/coworld/build-a-coworld/hosted-verification"
  >
    Check the deployed runtime and the evidence it produces.
  </Card>
  <Card
    title="Run mixed human/agent lobbies"
    href="/docs/coworld/build-a-coworld/league-lobbies"
  >
    Create league lobbies so humans and uploaded agents play one persisted
    episode with logs and the LLM sidecar.
  </Card>
</CardGroup>
