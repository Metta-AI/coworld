...
def test_refresh_resets_to_fetched_ref_for_branch_or_tag(tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
    spec = OptimizerRepoSpec(clone_url=DEFAULT_CLONE_URL, ref="v1.2.0", slug="metta-ai-optimizers")
    install_dir = tmp_path / spec.slug / spec.ref
    (install_dir / ".git").mkdir(parents=True)
    (install_dir / "package.json").write_text("{}", encoding="utf-8")
    calls: list[list[str]] = []
    ...
    assert runtime.ensure_optimizer_project(spec, tmp_path, refresh=True) == install_dir
    assert calls == [
...
def _patch_runtime(monkeypatch: MonkeyPatch) -> dict[str, object]:
    ...
    def fake_resolve_repository(manifest_path, *, override_repo=None, override_ref=None):
        spec = resolve_optimizer_repository(manifest_path, override_repo=override_repo, override_ref=override_ref)
