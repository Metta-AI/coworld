...
def test_manifest_requires_tags_field() -> None:
    manifest = _manifest_data()
    del manifest["tags"]
...
def test_manifest_rejects_legacy_container_reporter() -> None:
    """A container-shaped reporter ({id,name,type,image,...}) matches neither
    reference arm and is rejected. The backward-compat shim that used to drop
    these was removed once stored manifests were migrated off them (the data
    migration in 42044381d9ba); the parser is strict again per spec 0061."""
    data = _manifest_data()
    data["reporter"] = [
        {
            "id": "default-reporter",
