...
The campaign's most reusable finding is not any single dead end — it's the **descend-one-causal-
layer-and-re-measure** method that produced them. When copying a rival capability fails, do NOT
re-tune the same layer. Drop one layer down the causal stack and measure there:
...
are a layer that looked like the bottleneck and turned out to be downstream of a harder one.

A refutation is a result. Record the negative with its numbers so the lever is never rebuilt. This
campaign banked roughly ten refutations against two or three shipped wins; that ratio is normal.

**Verdict rule used throughout:** N≥30 per arm, Wilson confidence intervals. Overlapping CIs are
INCONCLUSIVE, never a win. 13%-vs-23%-scale effects need ~100–150 episodes/arm to separate. Pool
same-direction deficits across related configs only on the IDENTICAL config.
...
**Verdict:** any change whose mechanism is "vote/act more often" or "remember more candidates" must
measure accuracy/precision ALONGSIDE volume before shipping. More volume on the same evidence is a
loss.
