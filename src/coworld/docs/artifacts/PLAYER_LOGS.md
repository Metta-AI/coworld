...
For platform-hosted jobs, the runner reads up to the last 10,000 pod-log lines from started player containers. That is a
tail: a player that writes more than 10,000 lines in an episode keeps only the last 10,000, so the captured log loses
