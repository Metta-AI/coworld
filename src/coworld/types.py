...
class CoworldGameManifest(BaseModel):
    ...
    config_schema: JsonSchema = Field(
        description=(
            "JSON Schema for runtime game configs. It must require a fixed-length string-array `tokens` field; "
            "variants and certification configs omit tokens because the runner injects them."
        )
    )
