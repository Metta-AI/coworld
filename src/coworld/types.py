...
def _omit_default_from_schema(schema: JsonSchema) -> None:
...
class CoworldResourceLimits(BaseModel):
    ...
    cpu: str | None = Field(
        ...
        description=(
            ...
            "the node has, so the runnable behaves the same on every node size. For the player role, the backend "
            "also pins the container's math-library thread pools (OMP/MKL/OpenBLAS/NumExpr) to this many cores. "
            "Omit for no limit (the container may burst to the whole node)."
        ),
    )
    memory: str | None = Field(
        default=None,
        description=(
            "Kubernetes memory limit quantity, e.g. '3Gi'. Caps the container's memory regardless of node size. "
            "Today only the game role honors a memory limit. Omit for no limit."
        ),
    )
...
class CoworldRunnableResources(BaseModel):
    ...
    limits: CoworldResourceLimits | None = Field(
        default=None,
        description=(
            "Optional hard compute ceiling. Today the player role honors a CPU limit (also pinning its thread "
            "pools to match), and the game role honors both a CPU limit and a memory limit. Declaring a limit "
            "field a role does not honor is rejected at registration. Clamped to the role's limit bounds by the "
            "backend."
        ),
    )
...
class CoworldReplayViewer(BaseModel):
    """Opaque static replay-viewer bundle (spec 0068)."""
    ...
    bundle: str = Field(
        min_length=1,
        description=(
            "Package-relative static replay-viewer output directory generated by coworld build, then rewritten to a "
    ...
    def validate_bundle(cls, bundle: str) -> str:
        if re.fullmatch(REPLAY_VIEWER_BUNDLE_DIGEST_PATTERN, bundle):
            return bundle
        if bundle.startswith("sha256:"):
            raise ValueError("replay_viewer.bundle sha256 digest is malformed")
...
class CoworldGameManifest(BaseModel):
    ...
    replay_viewer: CoworldReplayViewer | None = Field(
        default=None,
...
class CoworldManifest(BaseModel):
    ...
    tags: Annotated[list[Annotated[str, Field(min_length=1)]], Field(min_length=3)] | SkipJsonSchema[None] = Field(
        ...
        json_schema_extra=_omit_default_from_schema,
    )
