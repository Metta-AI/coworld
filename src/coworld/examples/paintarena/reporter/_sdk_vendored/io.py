"""URI I/O helpers and env-var accessors for reporter implementations.

Implements the I/O surface every reporter in this repo needs:

- Environment-variable accessors for ``COGAME_REPORT_REQUEST``.
...
from .protocol import ReporterEpisodeInput, ReportRequest
...
class ReporterInputs(BaseModel):
    """The typed inputs every reporter is invoked with."""
...
def load_reporter_inputs() -> ReporterInputs:
    """Read the canonical report request env var. Raises ``KeyError`` if missing."""
    request = ReportRequest.model_validate_json(os.environ[REPORT_REQUEST_ENV_VAR])
    return ReporterInputs(episodes=request.episodes, report_uri=request.report_uri)
